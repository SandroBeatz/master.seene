...
const CATEGORIES = [
  'makeup', 'hair', 'nails', 'barber', 'massage',
  'tattoo_piercing', 'depilation', 'cosmetology', 'brows_lashes',
] as const

const isSubmitting = ref(false)

const enabledDays = computed(() =>
  DAY_KEYS.filter((day) => store.schedule.days[day].enabled),
)

function validate(): string | null {
  if (store.specializations.length === 0)
    return t('onboarding.step5.validation.missingSpecializations')
  if (!store.personal.firstName || !store.personal.lastName || !store.personal.phone || !store.personal.username)
...
async function createProfile() {
  ...
  try {
    ...
    if (error) {
      toast.add({ title: t('onboarding.step5.errorTitle'), description: error.message, color: 'error' })
      return
    }
...
<template>
  <div class="py-8 w-full space-y-5">
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-sm font-semibold text-default">{{ $t('onboarding.step5.sections.specializations') }}</span>
        <UButton
          variant="link"
          color="primary"
          size="xs"
      ...
      <div class="flex flex-wrap gap-1.5">
        <UBadge
          v-for="key in store.specializations"
          :key="key"
          color="primary"
          variant="soft"
        >
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-sm font-semibold text-default">{{ $t('onboarding.step5.sections.personal') }}</span>
        <UButton
          variant="link"
          color="primary"
          size="xs"
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-sm font-semibold text-default">{{ $t('onboarding.step5.sections.location') }}</span>
        <UButton
          variant="link"
          color="primary"
          size="xs"
      ...
      <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        ...
        <span>{{ store.location.canTravel ? $t('onboarding.step5.location.yes') : $t('onboarding.step5.location.no') }}</span>
      </div>
    </div>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-sm font-semibold text-default">{{ $t('onboarding.step5.sections.schedule') }}</span>
        <UButton
          variant="link"
          color="primary"
          size="xs"
      ...
      <p class="text-xs text-muted">{{ $t('onboarding.step5.schedule.timezone') }}: {{ store.schedule.timezone }}</p>
      <div v-if="enabledDays.length > 0" class="space-y-1">
        <div
          v-for="day in enabledDays"
          :key="day"
          class="text-sm flex flex-wrap gap-x-3 gap-y-0.5"
        >
          <span class="font-medium capitalize w-24">{{ $t(`onboarding.step4.days.${day}`) }}</span>
          <span class="text-muted">{{ store.schedule.days[day].start }} — {{ store.schedule.days[day].end }}</span>
          <span
            v-for="(brk, i) in store.schedule.days[day].breaks"
            :key="i"
            class="text-muted"
          >
