/**
 * Currency catalogue used across the app (price formatting, System & region
 * settings select).
 *
 * Currency is an app-level setting that is intentionally INDEPENDENT of the UI
 * locale: a master in Kazakhstan using the English interface still wants prices
 * in ₸. That's why we carry the symbol and its `position` here and format prices
 * manually (via `useFormats`) instead of relying on `Intl` currency formatting,
 * which would pick symbol/position from the locale.
 *
 * `decimals` is the default number of fraction digits shown for the currency.
 * CIS currencies are commonly displayed without minor units (0), western ones
 * with 2.
 */
...
/** Curated list: CIS + USA + Canada. KGS (сом, Kyrgyzstan) is mandatory. */
export const CURRENCIES: CurrencyOption[] = [
  ...
  { code: 'TJS', symbol: 'смн', position: 'suffix', decimals: 2, label: 'Tajikistani somoni (TJS)' },
